"""Provides unit tests for the SigninHandler class"""
...
class TestSigninHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        def get_user(u):
            return user.User.from_dict({"id": "1234", "userid": u.user_id})
        ...
        def setup_login_interactor():
            """Sets up the LoginInteractor mock"""

            def interactor_execute(u):
                return u.user_id == "validuser"

            return get_interactor(ui.LoginInteractor, interactor_execute)

        def setup_get_user_interactor():
            """Sets up the GetUserInteractor mock"""
            return get_interactor(ui.GetUserInteractor, get_user)
    ...
    def test_sets_interactor_hash_provider(self):
        """Test that calling SigninHandler.get_page causes LoginInteractor.set_hash_provider to 
        be called"""
    ...
    def test_login_successful_sets_cookies(self):
        ...
        for c in cs:
            self.__cookies.set_cookie.assert_any_call(c, cs[c])
    ...
    def test_missing_required_param_returns_failed_json(self):
        """Test that calling SigninHandler.get_page with a missing required parameter returns a 
        failed json object"""
    ...
    def test_empty_required_param_returns_failed_json(self):
        """Test that calling SigninHandler.get_page with an empty required parameter returns a
        failed json object"""
        j = json.dumps({'result': 'failed', 'message' : 'failed_validation'})
        required_params = ['userid', 'password']
