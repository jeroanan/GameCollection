...
class TestUpdateGameHandler(unittest.TestCase):
    ...
    def test_calls_interactor_execute(self):
        """Test that calling UpdateGameHandler.get_page causes UpdateGameInteractor.execute to be 
        called"""
        self.__target.get_page(params=self.__get_params())
        self.__interactor.execute.assert_called_with(
            game=Game.from_dict(self.__get_params()), user_id="1234")

    def test_misssing_required_params_returns_empty_string(self):
        """Test that any null required parameters causes UpdateGameHandler.get_page to return an 
        empty string"""
        self.__assert_operation_on_required_params_returns_empty_string(
            self.__missing_arg_returns_empty_string)

    def test_empty_required_params_returns_empty_string(self):
        """Test that any empty required parameters causes UpdateGameHandler.get_page to return an 
        empty string"""
        self.__assert_operation_on_required_params_returns_empty_string(
            self.__empty_arg_returns_empty_string)
    ...
    def test_persistence_exception_gives_empty_string(self):
        """Test that an exception from persistence causes UpdateGameHandler.get_page to return an 
        empty string"""
