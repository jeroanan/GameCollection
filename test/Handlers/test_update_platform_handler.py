...
class TestUpdatePlatformHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.__json_message_assertion = hta.get_params_returns_json_result_value_assertion(
            self,
            self.__target)
    ...
    def test_update_successful_returns_json_ok_message(self):
        """
        Test that calling UpdatePlatformHandler.get_page returns a json object with a result 
        value of 'ok'.
        """
    ...
    def test_none_platform_returns_json_validation_failed_message(self):
        """
        Test that calling UpdatePlatformInteractor.get_page with a None plaform returns a json 
        object with a result value of 'failed_validation'.
        """
        self.__json_message_assertion(None, 'validation_failed')

    def test_bad_name_gives_json_validation_failed_message(self):
        """
        Test that setting required fields to different bad values causes 
        DeletePlatformHandler.handler to return a json result of 'validation_failed'
        """
