...
class TestUpdatePlatformInteractor(itb.InteractorTestBase):
    ...
    def test_execute_calls_persistence_update_platform(self):
        """Test that calling UpdatePlatformInteractor.execute causes persistence.update_platform 
        to be called"""
    ...
    def test_execute_with_none_platform_raises_type_error(self):
        """Test that calling UpdatePlatformInteractor.execute with a null platform causes a 
        TypeError to be raised"""
        self.assertRaises(TypeError, self.__target.execute, None)

    def test_execute_validates_name_field(self):
        """Test that calling UpdatePlatformInteractor.execute causes platform.name to be 
        validated"""
        self.__target.execute(self.__platform)
    ...
    def test_execue_for_non_existing_platform_raises_platform_not_found_exception(self):
        """Test that if a platform with the given ID does not exist then
        PlatformNotFoundException is raised"""
