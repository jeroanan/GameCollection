...
class TestGetSuggestedPlatformsInteractor(InteractorTestBase):
    ...
    def setUp(self):
        super().setUp()
        self.__suggested_platforms = self.__get_suggested_platforms

        self.__target = platform_interactors.GetSuggestedPlatformsInteractor(
            self.__suggested_platforms)
    ...
    def test_execute_calls_persistence_get_platforms(self):
        """Test that calling GetSuggestedPlatformsInteractor.execute causes 
        persistence.get_platforms to be called"""
    ...
    def test_execute_returns_suggested_platforms_not_already_stored(self):
        """Test that calling GetSuggestedPlatformsInteractor.execute only returns platforms not 
        already stored in persistence"""
