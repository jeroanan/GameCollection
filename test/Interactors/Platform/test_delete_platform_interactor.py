...
class TestDeletePlatformInteractor(itb.InteractorTestBase):
    ...
    def test_execute_calls_delete_platform_persistence_method(self):
        """Test that calling DeletePlatformInteractor.execute causes persistence.delete_platform 
        to be called"""
        platform = p.Platform()
        platform.id = "id"
    ...
    def test_execute_with_none_platform_raises_type_error(self):
        """Test that calling DeletePlatformInteractor.execute with a null platform causes 
        TypeError to be raised"""
        self.assertRaises(TypeError, self.__target.execute, None)

    def test_execute_platform_does_not_exist_raises_platform_not_found_exception(self):
        """Test that when the platform to be deleted does not exist, PlatformNotFoundException 
        is raised"""
        platform = p.Platform()
