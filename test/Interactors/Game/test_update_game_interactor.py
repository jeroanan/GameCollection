...
class TestUpdateGameInteractor(InteractorTestBase):
    ...
    def test_calls_persistence_method(self):
        """Test that calling UpdateGameInteractor.execute causes persistence.update_game to be 
        called"""
    ...
    def test_with_null_game_raises_type_error(self):
        """Test that calling UpdateGameInteractor.execute with a null game causes TypeError to be 
        raised"""
        self.assertRaises(TypeError, self.__execute, None)

    def test_validates_title_field(self):
        """Test that UpdateGameInteractor.execute validates the game title field""" 
        self.__assert_string_validation("Game title", self.__game.title)

    def test_validates_platform_field(self):
        """Test that UpdateGameInteractor.execute validates the game platform field"""
    ...
    def test_raises_persistence_exception_when_database_throws(self):
        """Test that UpdateGameInteractor.execute raises PersistenceException when the
        persistence layer throws an exception"""
        def update_game(g):
            raise Exception
