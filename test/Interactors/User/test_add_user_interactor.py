...
class TestAddUserInteractor(unittest.TestCase):
    ...
    def test_execute_user_already_exists_raises_user_exists_exception(self):
        """Test that calling AddUserInteractor.execute with an existing user raises a 
        UserExistsException"""
    ...
    def test_execute_adds_new_user(self):
        """Test that calling AddUserInteractor.execute with valid parameters causes 
        persistence.add_user to be called"""
    ...
    def test_execute_calls_hash_provider(self):
        """Test that calling AddUserInteractor.execute with valid parameters causes user.passwod 
        to be hashed"""
        u = self.__get_user("new_user", "pass")
        passwd = u.password
        self.__target.execute(u)
