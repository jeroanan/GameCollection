"""Test DeleteHardwareTypeInteractor class"""
...
class TestDeleteHardwareTypeInteractor(unittest.TestCase):
    ...
    def test_execute_calls_persistence_delete_hardware_type_method(self):
        """
        Test that calling DeleteHardwareTypeInteractor.execute causes 
        persistence.delete_hardware_type to be called
        """
    ...
    def test_hardware_type_does_not_exist_raises_hardware_type_not_found_exception(self):
        """
        Test that when an attempt is made to delete a non-existant hardware type, 
        HardwareTypeNotFoundException is raised.
        """
        non_existant_hardware_type = self.__get_hardware_type(1415)
        self.assertRaises(
            hi.HardwareTypeNotFoundException,
            self.__target.execute,
            non_existant_hardware_type)

    def __get_hardware_type(self, hardware_id='1414'):
        h = {'id': hardware_id,
             'name': 'name',
