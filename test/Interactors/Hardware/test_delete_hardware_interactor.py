...
from Interactors.hardware_interactors import DeleteHardwareInteractor
...
class TestDeleteHardwareInteractor(InteractorTestBase):
    ...
    def test_execute_with_none_hardware_id_raises_type_error(self):
        """Test that calling DeleteHardwareInteractor.execute with a null hardwareid causes a 
        TypeError to be raised"""
        self.assertRaises(TypeError, self.__execute, None)

    def test_execute_validates_id_field(self):
        """Test that calling DeleteHardwareInteractor.execute causes the hardware id to be 
        validated"""
        self.__execute("id")
        self.validate_string_field_was_called_with("Hardware Id", "id")

    def test_execute_calls_persistence_method(self):
        """Test that calling DeleteHardwareInteractor.execute causes persistence.delete_hardware 
        to be called"""
