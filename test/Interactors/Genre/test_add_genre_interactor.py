...
class TestAddGenreInteractor(InteractorTestBase):
    ...
    def test_execute_with_none_genre_raises_type_error(self):
        """Test that calling AddGenreInteractor.execute with a null Genre causes a TypeError to 
        be raised."""
        self.assertRaises(TypeError, self.__target.execute, None)

    def test_execute_validates_title_field(self):
        """Test that AddGenreInteractor.execute validates the name field in the Genre it is 
        passed"""
        genre = Genre()
        genre.name = "Genre"
