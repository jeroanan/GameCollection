"""Assertion functions for handler unit tests"""
...
def get_empty_param_assertion(handler):
    """
    Gets an assertion function that tests an empty string is returned when a parameter is empty
    """
...
def get_missing_param_assertion(handler):
    """
    Gets an assertion function that tests an empty string is returned when a parameter is missing
    """
...
def get_params_returns_json_result_value_assertion(test_class, handler):
    """
    Get an assertion function that tests that calling a handler's get_page method causes the given
    result value of a json object to be returned.
    ...
    Returns:
        A function that will execute handler.get_page with the given parameters and assert that
        the expected value is returned in the resulting json object's result field.
    """

    def do_params_give_json_result_value(params, result_value):
        """
        Assert that when handler.get_page is called with params, the expected result_value is 
        returned in the returned json object's result field.
...
def get_exceptions_returns_json_result_value_assertion(test_class, handler, interactor):
    """
    Get an assertion that tests that when and exception is encountered by handler.get_page, the 
    expected result_value is returned for that exception.
    ...
    Returns:
        A function that will execute handler.get_page with the given parameters for each pair of 
        exception and result value and assert that the expected value is returned in the resulting 
        json object's result field when the given exception is encountered.
    """
    def do_exceptions_give_json_result_value(params, exceptions_and_results):
        """
        Assert that when handler is called with each of the given exceptions, the matching result 
        value is returned.
...
def get_exception_returns_json_result_value_assertion(test_class, handler, interactor):
    """
    Get an assertion function that tests that when an exception is encountered by handler.get_page, 
    the expected result_value is returned in the returned json object's result field.
    ...
    Returns:
        A function that will execute handler.get_page with the given parameters and assert that 
        the expected value is returned in the resulting json object's result field when the given 
        exception is encountered.
    """

    def does_exception_give_json_result_value(params, exception_type, result_value):
        """
        Assert that when handler.get_page encounters an exception of type exception_type, the 
        returned json object's result value is result_value
...
def get_bad_value_returns_json_validation_failed_assertion(test_class, handler, required_params):
    """
    Get an assertion function that tests that when required values are set to None/empty, 
    'validation_failed' is given in the returned json object's result field.
    ...
    Returns: A function that will execute handler.get_page with its required parameters set to 
    None/empty in turn. The returned json message's result field will be asserted to be 
    'validation_failed'
    """

    def do_bad_values_give_json_validation_failed_message(params):
        """
        Assert that calling handler.get_page with required parameters set to None/empty in turn 
        causes the returned json object's result field to be 'validation_failed'
...
def assert_operation_on_params_returns_true(func, params):
    """Assert that func returns true for each of the given params when they are set to None/empty 
    in turn"""
    for p in params:
        return func(p)
